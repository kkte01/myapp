...
@elseif($isTrashed and $hasChild)
    ...
    <div class="media item__comment {{ $isReply ? 'sub' : 'top' }}" data-id="{{ $comment->id }}" id="{{ $comment->id }}">
        ...
        <div class="media-body">
            ...
            <div class="action__comment">
                ...
                @endcan
                @if($currentUser)
                    <button class="btn__reply__comment">{{ trans('forum.comments.reply') }}</button>
                @endif
            </div>
            @if($currentUser)
                @include('comments.partial.create', ['parent_id'=>$comment->id])
            @endif
            @can('update', $comment)
